...
describe('ResumeController', () => {
  ...
  describe('GET /api/resumes/:resumeId/export', () => {
    test('should export resume as text', async () => {
      const mockBuffer = Buffer.from('John Doe\njohn@example.com');

      mockResumeService.exportResume.mockResolvedValue(mockBuffer);

      const response = await request(app)
        .get('/api/resumes/resume-123/export?format=txt')
        .expect(200);
    ...
    test('should export resume as DOCX', async () => {
      ...
      const response = await request(app)
        .get('/api/resumes/resume-123/export?format=docx')
        .expect(200);
    ...
    test('should export resume as PDF', async () => {
      ...
      const response = await request(app)
        .get('/api/resumes/resume-123/export?format=pdf')
        .expect(200);
    ...
    test('should return 400 for invalid export format', async () => {
      const response = await request(app)
        .get('/api/resumes/resume-123/export?format=invalid')
        .expect(400);
    ...
    test('should return 404 for non-existent resume', async () => {
      mockResumeService.exportResume.mockRejectedValue(new Error('Resume not found'));

      const response = await request(app)
        .get('/api/resumes/non-existent/export?format=txt')
        .expect(404);
